...
def api_checkout():
    ...
    settings = get_settings()
    wa_num = settings.get("whatsapp_number", "573123456789")
    numbers_str = ", ".join(order["tickets"][:10])
    if len(order["tickets"]) > 10:
        numbers_str += f" y {len(order['tickets']) - 10} más"

    wa_message = (
        f"¡Hola! Acabo de reservar {order['ticket_count']} boletas para el sorteo del iPhone 17 Pro.\n\n"
        ...
        f"Adjunto mi comprobante de pago para validación."
    )
...
def api_admin_export_excel():
    ...
    headers = [
        ...
        "Correo Electrónico", "Ciudad", "Cant. Boletas", "Total COP",
        "Estado", "Método de Pago", "Fecha Registro", "Números"
    ]
    ws.append(headers)
